...
// copied from AllureCucumber7Jvm
public class McckAllureCucumber7Jvm implements ConcurrentEventListener {
	...
	public McckAllureCucumber7Jvm() {
		this(Allure.getLifecycle());
	}

	public McckAllureCucumber7Jvm(final AllureLifecycle lifecycle) {
	...
	private void handleTestCaseStarted(final TestCaseStarted event) {
		...
		final McckAllureCucumber7JvmLabelBuilder labelBuilder = new McckAllureCucumber7JvmLabelBuilder(feature, testCase, tags);
	...
	private void handleTestStepStarted(final TestStepStarted event) {
		if (event.getTestStep() instanceof PickleStepTestStep) {
			...
			final String stepKeyword = Optional.ofNullable(
				testSources.getKeywordFromSource(currentFeatureFile.get(), pickleStep.getStep().getLine())).orElse("UNDEFINED");
	...
	private void handleHookStep(final TestStepFinished event) {
		...
		if (!Status.PASSED.equals(fixtureResult.getStatus())) {
			...
			if (hookStep.getHookType() == HookType.BEFORE) {
				final McckAllureCucumber7JvmTagParser tagParser = new McckAllureCucumber7JvmTagParser(currentFeature.get(), currentTestCase.get());
				statusDetails
	...
	private void handlePickleStep(final TestStepFinished event) {
		...
		if (event.getResult().getStatus() == io.cucumber.plugin.event.Status.UNDEFINED) {
			...
			lifecycle.updateTestCase(getTestCaseUuid(currentTestCase.get()), scenarioResult -> scenarioResult
